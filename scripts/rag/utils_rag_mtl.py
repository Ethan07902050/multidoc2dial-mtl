...
class Seq2SeqDataset(Dataset):
    def __init__(
        ...
        self.src_file = Path(data_dir['generation']).joinpath(type_path + ".source")
        self.domain_file = Path(data_dir['generation']).joinpath(type_path + ".domain")
        self.src_lens = self.get_char_lens(self.src_file)
        for task in TASKS:
            self.tgt_files = {
                task: Path(data_dir[task]).joinpath(type_path + ".target") for task in TASKS
            }        
    ...
    def __getitem__(self, index) -> Dict[str, Dict[str, torch.Tensor]]:
        ...
        for task in TASKS:        
            tgt_line = linecache.getline(str(self.tgt_files[task]), index).rstrip("\n")
            ...
            item[f'{task}_decoder_input_ids'] = target_ids
    ...
    def collate_fn(self, batch) -> Dict[str, Dict[str, torch.Tensor]]:
        # Source
        input_ids = torch.stack([x["input_ids"] for x in batch])
        masks = torch.stack([x["attention_mask"] for x in batch])
        token_type_ids = torch.stack([x["token_type_ids"] for x in batch])
        domain = [x[f"domain"] for x in batch]
        src_pad_token_id = (
        ...
        # Target
        for task in TASKS:
            target_ids = torch.stack([x[f"{task}_decoder_input_ids"] for x in batch])
            tgt_pad_token_id = (
            ...
            collated_data[f'{task}_decoder_input_ids'] = y
