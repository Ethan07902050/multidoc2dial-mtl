...
def evaluate_batch_e2e(args, rag_model, questions, domains=None):
    with torch.no_grad():
        ...
        answers = {
            task: rag_model.retriever.generator_tokenizer.batch_decode(outputs[task], skip_special_tokens=True)
            for task in TASKS
        }
...
def main(args):
    ...
    for checkpoint in checkpoints:
        ...
        eval_file = open(args.evaluation_set, "r")
        predictions_path = {task: os.path.join(args.model_name_or_path, f'{task}_preds.txt') for task in TASKS}
        preds_file = {
            task: open(predictions_path[task], "w") for task in TASKS
        }
        ...
        eval_file.close()
        for task in TASKS:
            preds_file[task].close()
        
        score_fn(args, predictions_path['grounding'], args.grounding_gold_data_path)
        score_fn(args, predictions_path['generation'], args.generation_gold_data_path)
