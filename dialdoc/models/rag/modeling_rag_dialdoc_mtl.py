...
class DialDocRagTokenForGenerationMTL(RagTokenForGeneration):
    ...
    def generate(
        ...
        **model_kwargs,
        ...
        if num_beams == 1:
            ...
            beam_scorer = {task: BeamSearchScorer(
    ...
    def beam_search(
        self,
        input_ids: torch.LongTensor,
        beam_scorer: Dict[str: BeamScorer],
        logits_processor: Optional[LogitsProcessorList] = None,
        stopping_criteria: Optional[StoppingCriteriaList] = None,
        max_length: Optional[int] = None,
        pad_token_id: Optional[int] = None,
        ...
        **model_kwargs,
        ...
        beam_score = torch.zeros((batch_size, num_beams), dtype=torch.float, device=input_ids.device)
        beam_score[:, 1:] = -1e9
        beam_score = beam_score.view((batch_size * num_beams,))
        beam_scores = {task: beam_score for task in TASKS}
        seq_ids = {task: input_ids for task in TASKS}
        stop = [0] * 2
        next_token_record, next_indices_record = {}, {}
        this_peer_finished = False  # used by synced_gpus only
        while True:
            ...
            for i, task in enumerate(TASKS):
                ...
                next_token_scores = next_token_scores + beam_scores[task][:, None].expand_as(next_token_scores)
                ...
                next_indices_record[task] = next_indices
                next_tokens_record[task] = next_tokens

                # stateless
                beam_outputs = beam_scorer[task].process(
                    seq_ids[task],
                    next_token_scores,
                    next_tokens,
                    next_indices,
                ...
                seq_ids[task] = torch.cat([seq_ids[task][beam_idx, :], beam_next_tokens.unsqueeze(-1)], dim=-1)
                ...
                if beam_scorer[task].is_done or stopping_criteria(seq_ids[task], scores):
                    stop[i] = 1
        ...
        sequence_outputs = {task: beam_scorer[task].finalize(
            seq_ids[task],
            beam_scores[task],
            next_tokens_record[task],
            next_indices_record[task],
            pad_token_id=pad_token_id,
            eos_token_id=eos_token_id,
            max_length=stopping_criteria.max_length,
        )["sequences"] for task in TASKS}
